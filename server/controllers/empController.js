...
async function getLoggedInEmployee(req, res) {
  try {
    const emp_id = req.id;
    const [result] = await db.execute(
      "SELECT full_name,location_name,latitude,longitude FROM employees JOIN branch_locations ON employees.branch_location_id=branch_locations.id WHERE employees.id= ?",
      [emp_id]
    );
    res.send({
...
async function addEmployee(req, res) {
  try {
    ...
    if (email.length > 0) {
      ...
      await db.execute(
        "INSERT INTO remaining_leaves (emp_id,annual_leaves,sick_leaves,casual_leaves) VALUES (?,?,?,?)",
        [result.insertId, 3, 3, 2]
      );
      res.send(result);
    }
  } catch (error) {
...
async function deleteEmployee(req, res) {
  try {
    ...
    const is_admin = Boolean(
      checkAdmin.find((item) => item.id.toString() === emp_id)
    );
...
async function generateReport(req, res) {
  ...
  var result=[];
  if(id===undefined){
     [result]= await db.execute('SELECT attendance.id,location_name,date,full_name,status,in_time,out_time,late_arrival,early_departure FROM attendance JOIN employees ON attendance.emp_id=employees.id JOIN branch_locations on employees.branch_location_id=branch_locations.id WHERE date between ? AND ? ORDER BY attendance.date DESC',[start,end])
  }
  else{
    [result]= await db.execute('SELECT attendance.id,location_name,date,full_name,status,in_time,out_time,late_arrival,early_departure FROM attendance JOIN employees ON attendance.emp_id=employees.id JOIN branch_locations on employees.branch_location_id=branch_locations.id WHERE employees.id = ? AND date between ? AND ? ORDER BY attendance.date DESC',[id,start,end])
  }
  ...
  const data = [  {name: 'John', location: 'New York', status: 'Present', in_time: '08:00 AM', out_time: '05:00 PM', late_arrival: 'No', early_departure: 'No', date: '2022-05-01'},  {name: 'Jane', location: 'London', status: 'Present', in_time: '09:00 AM', out_time: '06:00 PM', late_arrival: 'Yes', early_departure: 'No', date: '2022-05-01'}] // add more mock data here];

  const tableHeaders = ['Name', 'Location', 'Status', 'In Time', 'Out Time', 'Late Arrival', 'Early Departure', 'Date'];
...
result.forEach((row) => {
  ...
  rowData.push(DateTime.fromJSDate(row.date).setZone('Asia/Kolkata').toFormat('dd-MM-yyyy'));
